...
const NAV_ITEMS = [
  ...
  { to: "/mental-coach", label: "Mental Coach", icon: "M13 10V3L4 14h7v7l9-11h-7z" },
...
export default function Sidebar() {
  return (
    <nav className="w-64 shrink-0 bg-ink border-r border-border flex flex-col py-8 px-4 h-full">
      <div className="px-2 mb-10 flex items-center gap-3">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent2 to-accent flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.3)]">
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
        </div>
        <div>
          <p className="text-sm font-semibold text-white tracking-wide">COIN MANAGER <span className="text-accent">PRO</span></p>
          <p className="text-[10px] text-textdim uppercase tracking-wider font-medium">Comportement & Discipline</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {NAV_ITEMS.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              "flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 outline-none focus-visible:ring-2 focus-visible:ring-accent " +
              (isActive
                ? "bg-accent/10 text-accent shadow-[inset_2px_0_0_0_#10B981]"
                : "text-textdim hover:text-white hover:bg-panel2")
            }
          >
            {({ isActive }) => (
              <>
                <svg className={`w-5 h-5 ${isActive ? 'text-accent' : 'text-textdim'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
