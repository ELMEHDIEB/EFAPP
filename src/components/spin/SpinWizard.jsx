...
export function SpinWizard({ accounts, isPast, onComplete, onCancel }) {
  ...
  // Bypassed if we are logging a past spin
  const isHighRisk = (isDangerPostSpin || riskLabel !== "Rational" || exceedsWeeklyLimit) && !isPast;
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">
            {isPast ? "Ajouter un Ancien Spin" : "Nouveau Spin"}
          </h1>
          ...
          <div className="space-y-6">
            ...
            {isPast && (
              <label className="flex flex-col gap-2 mt-2">
                <span className="text-sm font-medium text-textdim">Date du tirage passé</span>
                <input 
                  type="date" 
                  value={customDate}
            ...
            {protectionStatus.isBelowThreshold && !isPast && (
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <input 
                  ...
                  onChange={e => {
                    ...
                    setCoinsSpent(val);
                    if (val && !isNaN(val)) setSpinsCount(Math.floor(Number(val) / 100).toString());
                  }}
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <input 
                  ...
                  onChange={e => {
                    const val = e.target.value;
                    setSpinsCount(val);
                    if (val && !isNaN(val)) setCoinsSpent((Number(val) * 100).toString());
                  }}
                  className="input py-3"
                />
              </label>
